...
echo "Setting DB secrets for Tekton Results"
kubectl create namespace tekton-results -o yaml --dry-run=client | oc apply -f-
if ! kubectl get secret -n tekton-results tekton-results-database &>/dev/null; then
  kubectl create secret generic tekton-results-database \
    --namespace="tekton-results" \
    --from-literal=DATABASE_USER=tekton \
    --from-literal=DATABASE_PASSWORD=$(openssl rand -base64 20)
fi
