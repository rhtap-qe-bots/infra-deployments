...
By default the bundle is defined in `components/build-service/build-pipeline-selector.yaml` namespace.

To change the default pipeline bundles, either update the default yaml file and provision cluster again, or create a `BuildPipelineSelector` CR in your own namespace. Refer to the `build-pipeline-selector.yaml` as an example.
...
| redhat-appstudio-user-workload | redhat-appstudio-user-workload secret in build-templates-e2e namespace | Quay secret allowing to push into default AppStudio repository | users/serviceaccounts with edit role |
| redhat-appstudio-staginguser | redhat-appstudio-staginguser secret in build-templates-e2e namespace | Quay secret allowing to push into component repositories in redhat-appstudio org | `pipeline` service accounts defined in [shared-resources-components.yaml](https://github.com/redhat-appstudio/infra-deployments/blob/main/components/build/shared-resources/shared-resources-components.yaml)|
