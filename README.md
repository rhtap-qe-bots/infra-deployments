...
```
$ ./hack/setup-monitoring.sh oauth2-secret prometheus-oauth2-proxy \
  $PROMETHEUS_GITHUB_CLIENT_ID \
  $PROMETHEUS_GITHUB_CLIENT_SECRET \
  $PROMETHEUS_GITHUB_COOKIE_SECRET

$ ./hack/setup-monitoring.sh oauth2-secret grafana-oauth2-proxy \
...
```
$ ./hack/setup-monitoring.sh grafana-datasource-secret \ 
  $DATASOURCE_NAME \
  $PROMETHEUS_URL \ 
  $TOKEN
```
...
```
$ oc get route/prometheus-k8s -n openshift-monitoring -o json | jq -r '.status.ingress[0].host

$ oc get route/prometheus -n appstudio-workload-monitoring -o json | jq -r '.status.ingress[0].host
```

`TOKEN` is obtained by requesting a token for the `grafana` service account in the **Prometheus cluster**:
```
$ oc create secret grafana -n appstudio-workload-monitoring
```
